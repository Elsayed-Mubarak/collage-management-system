...
public class AttendanceController {
	...
	/**
	 * @RequestMapping("/addattendance") public String
	 * addcourse(HttpServletRequest request) { request.setAttribute("mode",
	 * "MODE_ADDattendance");
	 * 
	 * request.setAttribute("students", studentServcie.getAllStudents());
	 * //request.setAttribute("secList", this.getWebFrameworkList()); //
	 * //request.setAttribute("a", a);
	 * //model.addAttribute("favoriteFrameworks", Attendance.ge)); return
	 * "welcomecoursepage"; }
	 **/
	@PostMapping("/attendance-saved")
	public String saveCourseIndataBase(@ModelAttribute Attendance attendance, BindingResult binding,
			HttpServletRequest request) {

		attendnceServcie.addAttendance(attendance);
		// request.setAttribute("mode", "ALL_ATTENDANCES");
		// request.setAttribute("attendance",
		// attendnceServcie.getAllAttendances());
		// request.setAttribute("secList", this.getWebFrameworkList());
	...
	/*
	 * @RequestMapping(value = "/attendance", method = RequestMethod.GET) public
	 * String addUser(//@ModelAttribute("SpringWeb")Attendance user, ModelMap
	 * model) { //model.addAttribute("username", user.getUsername());
	 * model.put("students", studentServcie.getAllStudents());
	 * model.put("attendance", new Attendance()); List<Attendance>
	 * atts=attendnceServcie.getAllAttendances(); // List<Student>
	 * studens=studentServcie.getAllStudents(); // for (Student att: studens) {
	 * // att.getAttendance(). //}
	 * 
	 * 
	 * for (Attendance att: atts) {
	 * 
	 * model.put("atts",att.getDone()); } // model.put("attendanceform", new
	 * attendanceform()); return "attendance"; }
	 */
	...
	public String User(@ModelAttribute(value = "attendance") Attendance attendance, ModelMap model) {
		/**
		 * List<Student> students= new ArrayList<>();
		 * 
		 * studentServcie.getAllStudents().forEach(students::add); for (Student
		 * s : students) { System.out.println("mosaaaaaa"); // {
		 ...
		 *                       public String
		 *                       addUser(@ModelAttribute("SpringWeb")Attendance
		 *                       user, ModelMap model) {
		 ...
		 *                       user.getAddress());
		 *                       model.addAttribute("receivePaper",
		 *                       user.isReceivePaper()); return "users";
		 **/
	}

	/**
	 * @RequestMapping("/editattendance") public String editCourse(@RequestParam
	 * int id,HttpServletRequest request) {
	 * request.setAttribute("course",courseServcie.editCourse(id));
	 * request.setAttribute("mode", "MODE_UPDATE"); return "welcomecoursepage";
	 * }
	 **/

	// @RequestMapping("/course-attendance")
	// public String deleteCourse(@RequestParam int id, HttpServletRequest
	// request) {
	...
	public String attendacebycourse(// @ModelAttribute("SpringWeb")Attendance
									// user,
			ModelMap model, @RequestParam String coursename, @RequestParam String secorder) {
		// model.addAttribute("username", user.getUsername());
		// model.put("attendance",
		// attendnceServcie.getAllAttendancesByCoursename(coursename));
	...
	//
	// model.put("attendance",
	// attendnceServcie.getexeededattendancecoursename(coursename));
	// // model.put("attendance",
	// attendnceServcie.getAllByCoursenameAndSecorder(coursename, secorder));
	// List<AggregateResults>
	// atts=attendnceServcie.getexeededattendancecoursename(coursename);
	...
	public String showattendanceCourses(@RequestParam String coursename, ModelMap model) {
		...
		// model.put("attendance",
		// attendnceServcie.getAllByCoursenameAndSecorder(coursename,
		// secorder));
	...
	public String attendacedetails(// @ModelAttribute("SpringWeb")Attendance
									// user,
	...
	public String totalattendacedetails(// @ModelAttribute("SpringWeb")Attendance
										// user,
			ModelMap model, @RequestParam String coursename, @RequestParam String studentname) {
